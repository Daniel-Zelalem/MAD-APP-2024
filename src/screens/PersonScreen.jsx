...
const PersonScreen = () => {
  ...
  const favoriteKey = `favorite_${userEmail}_${item.id}`;
  ...
  const retrieveFavoriteStatus = async () => {
    try {
      const favoriteStatus = await AsyncStorage.getItem(favoriteKey);
      if (favoriteStatus !== null) {
        setFavourite(JSON.parse(favoriteStatus));
      }
    } catch (error) {
  ...
  const showAlert = () => {
    Alert.alert(
      "Favorite Status",
      `Person ${
        isFavourite ? "Removed From MyFavorite" : "Added To MyFavorite"
      }.`
    );
  };
